...
if [ "$OXIDE" = "1" ]; then
  ...
  if [ ! -f "/steamcmd/rust/Compiler.x86_x64" ]; then
	...
	if [ "$INSTALL_OXIDE" = "1" ]; then
		...
		OXIDE_URL=$(curl -sL https://appveyorcidata.blob.core.windows.net/umod-18290/umod-rust/4281/xhmuptmemnstimnn/Oxide.Rust-linux.zip | grep browser_download_url | cut -d '"' -f 4)
